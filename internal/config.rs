...
fn load_dashboard_keyring() -> Result<Zeroizing<Vec<[u8; 32]>>> {
    // 1. File path first. Delegate parsing to the testable helper.
    let path = Path::new(DASHBOARD_KEYRING_PATH);
    if path.exists() {
        return load_dashboard_keyring_at(path);
    }
    ...
    if let Some(bytes) = load_key32_opt("DASHBOARD_VERIFY_KEY") {
        ...
        ring.push(bytes);
        seed_keyring_file_at(Path::new(DASHBOARD_KEYRING_PATH), &bytes)
            .context("seed dashboard-keyring on first boot")?;
        return Ok(ring);
    }
...
/// Read and parse the dashboard verify-keyring at `path`. Each
/// non-comment, non-blank line must be base64 encoding exactly 32 bytes.
/// On Unix, refuses to load if the file is world- or group-readable.
/// Returns `Err` on missing / unreadable / malformed content — caller
/// decides whether to fall back to env (production) or surface a test
/// assertion. Extracted from `load_dashboard_keyring` so tests can
/// drive it with a temp path instead of `/etc/glyndor/...`.
fn load_dashboard_keyring_at(path: &Path) -> Result<Zeroizing<Vec<[u8; 32]>>> {
    // Refuse to load if perms are wider than 0o600 — the audit
    // calls this out as M17 (secrets-at-rest). `metadata` can fail
    // on some filesystems; treat that as "skip the perm gate" so a
    // stripped filesystem still loads (the file's the source of truth).
    #[cfg(unix)]
    {
        if let Ok(meta) = std::fs::metadata(path) {
            ...
            if mode & 0o077 != 0 {
                anyhow::bail!(
                    "{} is world- or group-readable (mode {:o}); refusing to load. \
                     chmod 600 the file and restart.",
                    path.display(),
                    mode
                );
            }
    ...
    for (idx, line) in raw.lines().enumerate() {
        ...
        let bytes = Base64::decode_vec(trimmed)
            .with_context(|| format!("{}:{idx} not base64", path.display()))?;
        let arr: [u8; 32] = bytes.as_slice().try_into().map_err(|_| {
            anyhow::anyhow!(
                "{}:{idx} must decode to exactly 32 bytes, got {}",
                path.display(),
                bytes.len()
            )
        })?;
...
/// Best-effort write of a single 32-byte public key to `path` as base64
/// (mode 0o600 on Unix, atomic via `tmp + rename`). Used for seed-on-load
/// in production and tested directly with a temp path.
fn seed_keyring_file_at(path: &Path, bytes: &[u8; 32]) -> Result<()> {
    use std::io::Write;
    ...
    std::fs::rename(&tmp_path, path)
        .with_context(|| format!("rename {tmp_path} -> {}", path.display()))?;
    tracing::info!(
...
mod tests {
    ...
    fn decodes_to_31_bytes_rejected_with_length_message() {
        ...
        let err = load_dashboard_keyring_at(&path)
            .expect_err("b64-of-31-bytes must Err at the length gate");
        let msg = format!("{err:#}");
        assert!(
            msg.contains("must decode to exactly 32 bytes"),
    ...
    fn decodes_to_33_bytes_rejected_with_length_message() {
        ...
        let err = load_dashboard_keyring_at(&path)
            .expect_err("b64-of-33-bytes must Err at the length gate");
        let msg = format!("{err:#}");
        assert!(
            msg.contains("must decode to exactly 32 bytes"),
            "rejection must name the length control; got: {msg}"
    ...
    fn world_or_group_readable_perms_rejected() {
        ...
        let err =
            load_dashboard_keyring_at(&path).expect_err("0o644 keyring must Err before parse");
        let msg = format!("{err:#}");
        assert!(
    ...
    fn env_set_derives_single_32_byte_key() {
        ...
        let derived = load_key32_opt("DASHBOARD_VERIFY_KEY")
            .expect("env-set b64-of-32-bytes must derive a key");
        assert_eq!(
            derived, key,
    ...
    fn load_secret_opt_returns_some_when_env_set() {
        ...
        let got =
            load_secret_opt("HELMLY_TEST_LOAD_SECRET_OPT").expect("env-set must produce Some(_)");
